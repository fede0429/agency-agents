...
function clearStoredTokens() {
  for (const key of ['ugcvp_access_token', 'ugcvp_refresh_token', 'token']) {
...
async function loadCurrentUser() {
  ...
  const testLink = document.getElementById('nav-test-link');
  const token = getAccessToken();
  if (!token) {
  ...
  try {
    ...
    if (navUser) {
      ...
      navUser.textContent = `${label}${me.role === 'admin' ? ' · 管理员' : ''}`;
    }
    const isAdmin = me.role === 'admin';
    adminLink?.classList.toggle('hidden', !isAdmin);
    testLink?.classList.toggle('hidden', !isAdmin);
    return me;
  } catch (err) {
    clearStoredTokens();
    if (navUser) navUser.textContent = '登录失效';
    location.href = '/login.html';
    return null;
  }
}
