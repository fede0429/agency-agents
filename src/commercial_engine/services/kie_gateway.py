...
ELEVENLABS_VOICE_NAME_MAP = {
    # Common ElevenLabs default voice IDs used in existing config.
...
class KieGateway:
    ...
    async def tts(
        ...
        """
        Backward-compatible TTS wrapper for older UGC services.

        KIE's ElevenLabs TTS models use the generic createTask endpoint with
        text plus voice metadata. Older callers pass `voice_id`; we forward it
        as multiple compatible keys so existing config keeps working.
        """
        voice_name = ELEVENLABS_VOICE_NAME_MAP.get(voice_id, voice_id)
        input_params = {
            "text": text,
            "voice": voice_name,
            "voice_id": voice_id,
        ...
        task = await self.create_task(model, input_params, provider=KieProvider.GENERIC)
        ...
        audio_url = result.result_urls[0] if result.result_urls else ""
        if not audio_url:
            raise RuntimeError(f"KIE TTS task {task.task_id} returned no audio URL")
    ...
    async def download_file(
        ...
        """Backward-compatible alias used by legacy TTS code."""
        return await self.download(url, output_path, timeout=timeout)

    async def upload_file(
        self,
        file_path: str,
        upload_path: str = "ugc-uploads",
    ) -> str:
        """
        Upload a local file to KIE.AI CDN and get an HTTPS URL back.

        KIE's file-base64-upload endpoint accepts arbitrary file payloads, so we
        reuse it for lipsync audio as well as image assets.
        """
        ...
        with open(file_path, "rb") as f:
            file_bytes = f.read()
        file_b64 = b64_mod.b64encode(file_bytes).decode()
        ...
        base64_data = f"data:{mime};base64,{file_b64}"
        ...
        logger.info(
            f"KIE upload_file: {path.name} ({len(file_bytes) / 1024:.1f} KB)"
        )
        ...
        logger.info(f"KIE upload_file success: {download_url[:80]}...")
        return download_url
    ...
    async def generate_video(
        ...
        """
        Backward-compatible video generation wrapper used by older UGC code.

        Newer code routes KIE video generation through KieVideoAdapter. This
        wrapper keeps older callers working without duplicating the model
        registry and provider-specific input builders here.
        """
        from models.kie_video import KieVideoAdapter

        adapter = KieVideoAdapter(self.config, model_variant=model)
        job_id = await adapter.generate(
            prompt=prompt,
            duration=duration,
        ...
        retries = max_retries or self._max_retries
        logger.info(
            f"KIE generate_video compat: model={model}, job_id={job_id}, "
            f"duration={duration}s, has_ref={bool(image_path)}"
        )

        last_status = "pending"
        for attempt in range(1, retries + 1):
            job = await adapter.poll_status(job_id)
            last_status = job.status

            if job.status == "succeeded":
                if not job.video_url:
                    raise RuntimeError(
                        f"KIE job {job_id} succeeded without a result URL"
                    )

                target_path = output_path or str(
                    Path("tmp") / f"kie_{model}_{int(time.time())}.mp4"
                )
                video_path = await self.download(job.video_url, target_path)
                return {
                    "job_id": job_id,
                    "task_id": job_id.split(":", 1)[-1],
                    "status": job.status,
            ...
            if job.status == "failed":
                raise RuntimeError(
                    f"KIE job {job_id} failed: {job.error or 'Unknown error'}"
                )

            logger.debug(
                f"KIE generate_video compat poll: job_id={job_id}, "
                f"status={job.status}, attempt={attempt}/{retries}"
            )
            await asyncio.sleep(interval)

        raise TimeoutError(
            f"KIE job {job_id} timed out after {retries} polls "
            f"with last status={last_status}"
        )
