...
LIPSYNC_MODELS = {
    "kling_avatar_720p":  {"kie_model": "kling/ai-avatar-standard", "max_duration": 15, "price_per_sec": 0.04},
    "kling_avatar_1080p": {"kie_model": "kling/ai-avatar-pro", "max_duration": 15, "price_per_sec": 0.08},
...
class LipSyncService:
    ...
    async def generate(
        ...
        input_params = {
            "prompt": (
                "Create a natural talking-avatar video that preserves facial identity, "
                "matches the supplied speech timing closely, and keeps motion clean and stable."
            )
        }

        # Upload the face reference so KIE can fetch it reliably.
        if face_image.startswith(("http://", "https://")):
            input_params["image_url"] = face_image
        else:
            input_params["image_url"] = await self._gateway.upload_image(face_image)

        # Upload the narration audio to the same CDN flow.
        if audio_path.startswith(("http://", "https://")):
            input_params["audio_url"] = audio_path
        else:
            input_params["audio_url"] = await self._gateway.upload_file(
                audio_path,
                upload_path="ugc-audio",
            )
